...
The approved PNG is the full Skylink Bingwa lock-up on a white canvas. The production
...
artwork already contains the Skylink Bingwa wordmark, so do not add separate text to
it.

### `android`

- `skylink-bingwa-launcher-master-1024.png` — legacy launcher master.
- `skylink-bingwa-play-store-512.png` — Google Play listing icon.
- `skylink-bingwa-splash-mark-512.png` — Android splash-screen mark.
...
5. Reference `@mipmap/ic_launcher` and `@mipmap/ic_launcher_round` from the
   manifest.
6. Use `ic_stat_skylink_bingwa` as the notification small icon.
